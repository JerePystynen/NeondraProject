...
inline void startStreaming(WiFiClient *client)
{
  ...
  if ((!client->connected()) || (millis() >= ultimeout)) {
    Serial.println("Error filling buffer or client disconnected.");
    return;
  }
  Serial.println("Buffer filled successfully.");
  ...
  while (client->available() || (millis() < ultimeout) || (bufferPtrOut != bufferPtrIn)) {
    ...
    if (bufferPtrOut != bufferPtrIn) ultimeout = millis() + 500;
