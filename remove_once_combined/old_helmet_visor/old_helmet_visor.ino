...
const uint8_t kMatrixWidth = 76; // 8+8+32+32
...
void setup() {
  ...
  FastLED.setBrightness(getMatrixBrightness());
}
...
void loop() {
    ...
    if (ms < 5000) {
      FastLED.setBrightness(scale8(getMatrixBrightness(), (ms * 256) / 5000));
    } else {
      FastLED.setBrightness(getMatrixBrightness());
    }
    FastLED.show();
...
float getMatrixBrightness() {
  float sensorValue = analogRead(A0);
  float mb = sensorValue > 40 ? sensorValue : 0;
  return clamp(MAX_BRIGHTNESS * (mb / 1015), 0, MAX_BRIGHTNESS);
}
